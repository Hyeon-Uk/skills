#!/usr/bin/env bash
# gemini_image.sh — Google Gemini image generation for generate-ai-video.
# Adapted from generate-ai-image/scripts/gemini_generate.sh.
#
# Endpoint and model are STATIC, pinned to gemini-3.1-flash-image-preview.
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --) shift; PROMPT="$*"; break ;;
        *)  echo "gemini_image.sh: unexpected arg: $1" >&2; exit 1 ;;
    esac
done

API_KEY="$(get_yaml_nested3 providers gemini api_key "$CONFIG_FILE")"
if [ -z "$API_KEY" ]; then
    echo "gemini_image.sh: providers.gemini.api_key not found in $CONFIG_FILE" >&2
    echo "Set providers.gemini.api_key in $CONFIG_FILE before invoking this skill." >&2
    exit 1
fi

# Map WxH to a Gemini imageConfig.aspectRatio.
ASPECT="1:1"
case "$SIZE" in
    *x*)
        W="${SIZE%x*}"; H="${SIZE#*x}"
...
HTTP_CODE="$(curl -sS -w '%{http_code}' -o "$RESPONSE_FILE" \
    ...
    echo "gemini_image.sh: curl failed talking to $GEMINI_IMAGE_ENDPOINT" >&2
    exit 1
}

if [ "$HTTP_CODE" != "200" ]; then
    echo "gemini_image.sh: Gemini API returned HTTP $HTTP_CODE" >&2
    cat "$RESPONSE_FILE" >&2
    echo >&2
    exit 1
fi
...
if [ -z "$B64" ]; then
    echo "gemini_image.sh: could not extract image data from response" >&2
    cat "$RESPONSE_FILE" >&2
    exit 1
fi
...
if [ -n "$ACTUAL_EXT" ]; then
    ...
    if [ "$ACTUAL_EXT" != "$REQUESTED_EXT" ]; then
        ...
        echo "gemini_image.sh: response is $RETURNED_MIME; saving as $NEW_OUTPUT" >&2
        OUTPUT="$NEW_OUTPUT"
    fi
fi

printf '%s' "$B64" | base64 -d > "$OUTPUT" || {
    echo "gemini_image.sh: base64 decode failed" >&2
    exit 1
}
